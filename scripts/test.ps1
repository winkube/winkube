...
$env:GOOS = 'windows'
$env:CGO_ENABLED = 1
$LINKFLAGS = ('-X github.com/{0}/{1}/pkg/defaults.AppVersion={3} -X github.com/{0}/{1}/pkg/defaults.AppCommit={4} -linkmode external' -f $env:REPO, $env:PROG, $env:VERSION, $env:COMMIT)
ginkgo --ldflags $LINKFLAGS --randomizeAllSpecs --randomizeSuites --noisyPendings=false --noisySkippings=false --cover --coverpkg=github.com/$env:REPO/$env:PROG/... --trace --race -r tests/validation/
if (-not $?)
{
