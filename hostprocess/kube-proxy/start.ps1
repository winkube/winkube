...
if (test-path env:KUBEPROXY_PATH){ 
    ...
    $kproxy = "$env:CONTAINER_SANDBOX_MOUNT_POINT/bin/kube-proxy.exe"
}
...
while (-Not (Get-HnsNetwork | Where-Object Name -EQ $NetworkName)) {
...
Write-Output "kubeproxy version $kubeProxyVer"
...
mkdir -force /var/lib/rancher/kube-proxy/
((Get-Content -path $env:CONTAINER_SANDBOX_MOUNT_POINT/var/lib/rancher/agent/calico.kubeconfig -Raw) -replace '/var',"$($env:CONTAINER_SANDBOX_MOUNT_POINT)/var") | Set-Content -Path $env:CONTAINER_SANDBOX_MOUNT_POINT/var/lib/rancher/agent/calico.kubeconfig
Copy-Item $env:CONTAINER_SANDBOX_MOUNT_POINT/var/lib/rancher/agent/calico.kubeconfig /var/lib/rancher/agent/calico.kubeconfig
...
$argList = @(`
    ...
    "--kubeconfig=$env:CONTAINER_SANDBOX_MOUNT_POINT/var/lib/rancher/agent/calico.kubeconfig"`
)
$extraFeatures = @()
...
if ($network.Type -EQ "Overlay") {
    ...
    while (-Not (Get-HnsEndpoint | Where-Object Name -EQ "Calico_ep")) {
        Start-Sleep 1
    }
    Write-Host "Host endpoint found."
    $sourceVip = (Get-HnsEndpoint | Where-Object Name -EQ "Calico_ep").IpAddress
    $argList += "--source-vip=$sourceVip"
    $extraFeatures += "WinOverlay=true,IPv6DualStack=false"
}
